...
create_var() { # name value — создаёт или обновляет (delete+create) переменную n8n
  local name="$1" value="$2" resp id
  echo "≫ Variable $name"
  id="$(api GET "/variables?limit=100" | jq -r --arg k "$name" '.data[]? | select(.key == $k) | .id' | head -1 || true)"
  if [ -n "$id" ]; then
    curl -sS -o /dev/null -X DELETE "${AUTH[@]}" "$API/variables/$id" || true
  fi
  resp="$(curl -sS -o /dev/null -w "%{http_code}" -X POST "${AUTH[@]}" \
    -d "{\"key\":\"$name\",\"value\":\"$value\"}" "$API/variables" || true)"
  case "$resp" in
    2*) echo "  задана ✓" ;;
    *)  echo "  не удалось (HTTP $resp) — создайте вручную на странице /variables вашего n8n" ;;
  esac
}
